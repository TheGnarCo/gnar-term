...
| **Workspace** (entry)   | The top-level entry in the Workspaces sidebar section. A `WorkspaceEntry` is a user-created, path-rooted, colored grouping of workspaces. Renders inline as a banner with a nested list of its workspaces, and owns a Dashboard workspace hosting a markdown Live Preview. Managed by the workspace-groups extension. (Formerly: "Workspace Group" / "Project"; see ADR 004.) |
