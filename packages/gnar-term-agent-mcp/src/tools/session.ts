...
export function registerSessionTools(
  ...
  server.registerTool("spawn_agent", {
    ...
    const opts = args as SpawnOptions;
    ...
    if (opts.agent === "custom") {
      ...
      commandStr = AGENT_COMMANDS[opts.agent as AgentType].join(" ");
    }
  ...
  server.registerTool("kill_session", {
    description: "Terminate an agent session. Sends the specified signal to the process.",
    inputSchema: {
