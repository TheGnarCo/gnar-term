...
fn matches_segment(pattern: &str, segment: &str) -> bool {
    ...
    for (i, part) in parts.iter().enumerate() {
        ...
        if i == 0 {
            ...
            if seg.len() < part_bytes.len()
                || !seg[seg.len() - part_bytes.len()..].starts_with(part_bytes)
            {
                return false;
            }
            pos = seg.len();
...
fn collect_files_recursive(dir: &Path, base: &Path, files: &mut Vec<PathBuf>) {
    ...
    for entry in entries.flatten() {
        // `file_type()` does not follow symlinks; skipping links prevents
        // exfiltration via a link inside `source_dir` pointing at blocked
        // directories (e.g. ~/.ssh) and avoids traversal loops.
...
pub async fn copy_files(
    ...
    // Validate paths against sensitive directories before any file operations
    crate::validate_read_path(&source_dir)?;
    crate::validate_write_path(&dest_dir)?;
    ...
    for rel_path in &all_files {
        ...
        let matched = patterns
            .iter()
            .any(|pat| matches_glob(pat, &rel_normalized));
        if !matched {
            continue;
        }
...
mod tests {
    ...
    async fn copy_files_basic() {
        ...
        let dst = config_tempdir().join("dst");
    ...
    async fn copy_files_nonexistent_source() {
        ...
        let dst = format!("{home}/.config/gnar-term/test_dst");
        ...
        assert!(
            err.contains("does not exist") || err.contains("Invalid path"),
    ...
    #[tokio::test]
    async fn copy_files_rejects_dest_outside_config() {
        let tmp = tempdir();
        let src = tmp.join("src");
        fs::create_dir_all(&src).unwrap();
        fs::write(src.join("a.txt"), "a").unwrap();
    ...
    // Regression: collect_files must not follow symlinks (review #3) —
    // protects against exfiltration through links inside source_dir that
    // point at sensitive directories (e.g. ~/.ssh).
    #[cfg(unix)]
    #[tokio::test]
    async fn copy_files_skips_symlinks() {
        let tmp = tempdir();
        let src = tmp.join("src");
        let dst = config_tempdir().join("dst");
        ...
        // A symlink that points somewhere outside the source tree — the
        // contents of the target must never be copied.
        let outside = tmp.join("outside_target.txt");
        fs::write(&outside, "should-not-copy").unwrap();
        ...
        assert_eq!(count, 1, "only the real file should be copied");
        assert!(dst.join("real.txt").exists());
        assert!(
            !dst.join("linked.txt").exists(),
            "symlinks inside source_dir must not be followed"
        );
    }
    ...
    async fn run_script_echo() {
        ...
        let result = run_script(tmp.to_string_lossy().to_string(), "echo hello".to_string())
            .await
            .unwrap();
    ...
    async fn run_script_pwd() {
        ...
        let result = run_script(tmp.to_string_lossy().to_string(), "pwd".to_string())
            .await
            .unwrap();
    ...
    async fn run_script_nonzero_exit() {
        ...
        let result = run_script(tmp.to_string_lossy().to_string(), "exit 42".to_string())
            .await
            .unwrap();
    ...
    async fn run_script_cwd_is_file() {
        ...
        let result = run_script(file.to_string_lossy().to_string(), "echo hello".to_string()).await;
    ...
    fn tempdir() -> PathBuf {
        // Use thread ID + nanos to avoid collisions between parallel tests
        ...
        // Clean up any stale directory from a prior run
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        dir
    }

    /// Create a temp dir under ~/.config/gnar-term/ for tests that need
    /// to pass `validate_write_path`.
    fn config_tempdir() -> PathBuf {
        let home = std::env::var("HOME").expect("HOME env var required for tests");
        let dir = PathBuf::from(format!(
            "{home}/.config/gnar-term/test_{:?}_{}",
            std::thread::current().id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
