...
mod tests {
    ...
    fn setup_test_repo() -> std::path::PathBuf {
        ...
        let dir = std::env::temp_dir().join(format!("gnar-term-test-{}-{}", std::process::id(), id));
    ...
    async fn test_git_clone_empty_url() {
        let result = git_clone("".to_string(), "/tmp/target".to_string()).await;
        assert!(result.is_err());
    ...
    async fn test_push_branch_default_remote() {
        ...
        assert!(
            ...
            "Expected error about origin remote, got: {}",
            err
        );
    ...
    async fn test_push_branch_custom_remote() {
        ...
        assert!(
            ...
            "Expected error about upstream remote, got: {}",
            err
        );
        cleanup_test_repo(&repo);
    }
