...
describe("Extension management", () => {
  ...
  describe("installExtensionFromPath", () => {
    ...
    it("returns success: false when registerFn throws during activation", async () => {
      // This extension manifest is valid, but its entry file exports a default
      // function that throws on register. That counts as activation failure.
      // Expected: install result is an explicit failure the UI can surface.
      const manifest = JSON.stringify({
        id: "boom-ext",
        name: "Boom",
        version: "1.0.0",
      ...
      // Spy on registerExtension so we can simulate a registration throw —
      // the real dynamic-import path isn't easy to provoke in unit tests,
      // but the failure mode we care about is "registered-but-not-activated"
      // which registerExtension re-throws.
      const loader = await import("../lib/services/extension-loader");
      const spy = vi
        .spyOn(loader, "activateExtension")
        .mockRejectedValue(new Error("activation boom"));

      const result = await installExtensionFromPath("/path/to/boom-ext");

      expect(result.success).toBe(false);
      expect(result.error).toMatch(/activation boom/);

      // Error store should also have received the failure for the user toast path.
      const errors = get(loader.extensionErrorStore);
      expect(
        errors.some(
          (e) => e.id === "boom-ext" && /activation boom/.test(e.error),
        ),
      ).toBe(true);
  ...
  describe("loadExternalExtensions", () => {
    ...
    it("reports load failures to extensionErrorStore (H1)", async () => {
      mockInvoke.mockImplementation((cmd: string, args?: { path?: string }) => {
      ...
      vi.spyOn(config, "getConfig").mockReturnValue({
        extensions: {
          "broken-ext": {
            enabled: true,
            source: "local:/path/to/broken-ext",
          },
        },
      });
      ...
      expect(
        errors.some(
          (e) => e.id === "broken-ext" && /disk read failed/.test(e.error),
        ),
      ).toBe(true);
    });
