...
describe("appendRootRow", () => {
  ...
  it("treats nested-workspace:X and workspace:X as distinct rows", () => {
    appendRootRow({ kind: "nested-workspace", id: "shared" });
...
describe("moveRootRow", () => {
  it("reorders items, accounting for the shift when moving forward", () => {
    ...
    appendRootRow({ kind: "nested-workspace", id: "w1" });
    appendRootRow({ kind: "nested-workspace", id: "w2" });
    appendRootRow({ kind: "nested-workspace", id: "w3" });
    appendRootRow({ kind: "nested-workspace", id: "w4" });
    moveRootRow(0, 2);
...
describe("bootstrapRootRowOrder", () => {
  ...
  it("drops entries whose referent no longer exists", () => {
    ...
    expect(get(rootRowOrder)).toEqual([{ kind: "nested-workspace", id: "w1" }]);
  });

  it("appends newly-known entities (workspaces before unclaimed nestedWorkspaces) for first-run installs", () => {
    mockState = {};
