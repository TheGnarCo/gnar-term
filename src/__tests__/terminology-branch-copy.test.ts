 ...
 * for the nested workspace level, not "Workspace".
 *
...
describe("S8 terminology: Workspace → Branch in user-facing copy", () => {
  describe("init-workspaces.ts command palette titles", () => {
    const src = readSrc("bootstrap/init-workspaces.ts");

    it('command title does not say "New Workspace"', () => {
      expect(src).not.toContain("New Workspace");
    });

    it('command title does not say "Promote Workspace to Workspace"', () => {
      expect(src).not.toContain("Promote Workspace to Workspace");
    });
