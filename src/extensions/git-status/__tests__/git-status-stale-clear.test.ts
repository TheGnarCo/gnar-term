...
describe("git-status stale-data clear (H4)", () => {
  ...
  it("clears branch+dirty items when git status fails after a successful fetch", async () => {
    ...
    try {
      ...
      // Trigger another refresh via the title-changed debounce path.
      eventBus.emit({
      ...
      // Debounce is 2s — advance exactly that far so we don't also fire the
      // 30s polling intervals (which would re-run with the fail mock anyway).
