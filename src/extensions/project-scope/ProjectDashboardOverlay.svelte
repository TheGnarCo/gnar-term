<script lang="ts">
  ...
  import { PROJECT_COLORS, dashboardProjectId$ } from "./index";
  ...
  // Re-read project data when workspaces change (projects are stored in api.state)
  $: {
    void $workspacesStore;
    if (dashboardProjectId) {
      project = (api.state.get<ProjectEntry[]>("projects") ?? []).find(
        (p) => p.id === dashboardProjectId,
      );
    }
  }
...
{#if visible && project}
  ...
  >
    ...
    >
      ...
      >
        <div
          style="width: 12px; height: 12px; border-radius: 50%; background: {project.color}; flex-shrink: 0;"
        ></div>
        ...
        {#if activeTab === "overview"}
          ...
          <div style="display: flex; flex-direction: column; gap: 16px;">
            <div style="display: flex; align-items: center; gap: 8px;">
              <div
                style="width: 10px; height: 10px; border-radius: 50%; background: {project.color};"
              ></div>
              <div
            ...
            {#each projectWorkspaces as ws (ws.id)}
              ...
              <div
                style="
                  ...
                  ? project.color
                  : `color-mix(in srgb, ${project.color} 30%, transparent)`};
                "
