<script lang="ts">
   ...
   */
  export let kind: "parent" | "child" | "dashboard" = "child";

  /**
   * Density modifier for `kind: "child"`. When the row is rendered
   * inside a parent banner, callers pass `compact={true}` so its
   * vertical padding collapses to 0 — matches dashboard density and
   * prevents the child list from "breathing" inside the banner.
   * Ignored for `kind: "parent"` (banner controls its own padding)
