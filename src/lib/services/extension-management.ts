...
export async function installExtensionFromPath(
  ...
  try {
    ...
    activationError = String(err);
    reportExtensionError(
      manifest.id,
      `Installed but activation failed: ${activationError}`,
    );
  }
  ...
  extensions[manifest.id] = {
    enabled: activationError === undefined,
    source: `local:${dirPath}`,
  };
  ...
  for (const [extId, extConfig] of Object.entries(extensions)) {
    ...
    if (extConfig.source.startsWith("local:")) {
      ...
      try {
        ...
        // Surface to the error store so the user sees a toast on boot instead
        // of a silent failure. Flip config.enabled=false so we don't retry the
        // same broken extension on every launch.
        reportExtensionError(extId, `Failed to load from ${dirPath}: ${err}`);
        const disableCfg = getConfig();
        const disableExts = { ...disableCfg.extensions };
        if (disableExts[extId]) {
          disableExts[extId] = { ...disableExts[extId], enabled: false };
          await saveConfig({ extensions: disableExts });
        }
      }
    } else {
      console.warn(
