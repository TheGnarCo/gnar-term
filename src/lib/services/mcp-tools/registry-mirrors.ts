...
export const registryMirrorTools: ToolDef[] = [
  ...
  // ---- Dashboard contributions (per-workspace add / remove / list) ----
  {
    ...
    description:
      "List every registered Dashboard contribution. When `workspace_id` is provided, each row also carries `active` (whether the workspace has a dashboard child workspace for this contribution) and `branched_workspace_id` when active. `autoProvision` contributions cannot be added or removed; the Settings dashboard toggle surfaces this via `locked_reason`.",
    inputSchema: {
      type: "object",
      properties: {
        workspace_id: {
